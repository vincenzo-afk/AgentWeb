...
| Event-driven workflows | `POST /workflows` registers an organization-scoped task template for monitor change events; matching changes create durable `workflow_run` jobs, the supervised worker renders the template and executes a grounded solve, and redacted run records are available through `GET /workflows/runs`. Queue leases, retries, rate limits, and dead-letter behavior reuse the scheduler. |
...
The source roadmap is [`docs/roadmap.md`](docs/roadmap.md). The current implementation covers the dependency-free core of the Phase 0 baseline, several Phase 1 foundations, a Phase 2 graph storage/query slice, durable event-driven workflow jobs, connector-aware routing, deterministic vector retrieval, and a privacy-safe learning loop, including bounded durable crawling, scheduled monitoring, process-isolated browser execution, encrypted reusable browser session state, tenant-scoped graph entities/relations, corroboration-aware graph confidence, monitor-triggered task templates, queued workflow runs with scheduler retries, semantic skill fallback, cautious entity resolution, automatic solve outcome recording, and aggregate strategy summaries. Future phases cover richer external event integrations, operator controls, and calibrated production evaluation.
