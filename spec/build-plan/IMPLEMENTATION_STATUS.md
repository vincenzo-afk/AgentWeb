...
| Extractor | Partial | `AgentWebEngine.extract` returns parsed metadata, links, warnings, trust score, overall confidence, explainable confidence reasons, field-level confidence, and schema-guided normalized fields with deterministic normalization confidence. | Add richer tables/entities and source-span evidence when parser contracts expand. |
