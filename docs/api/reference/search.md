...
The default provider is the free DuckDuckGo HTML adapter. Set `AGENTWEB_SEARCH_PROVIDER=json` and `AGENTWEB_SEARCH_ENDPOINT` to use an HTTP JSON provider that returns either a result array or `{ "results": [...] }`; `AGENTWEB_SEARCH_API_KEY` is resolved through the external secret boundary when present. Result dates are accepted from `published_at`, `published_date`, or `date` and normalized to the public `published_at` field. If a configured JSON provider fails, AgentWeb falls back to DuckDuckGo and returns an empty list only when both providers are unavailable. Provider credentials and response bodies are never written to traces.
