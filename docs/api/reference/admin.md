...
List responses from `/admin/keys` and `/admin/audit` include `data`, `next_cursor`, and `has_more`; pass the returned cursor unchanged to request the next page. Local maintenance retains snapshots and crawl history for 90 days by default, traces and metrics for 30 days, and audit events for 730 days. Run `agentweb gc --crawl-days N --audit-days N` for immediate cleanup, or add `--schedule --run-at UNIX_TIMESTAMP` to enqueue one bounded `retention_gc` job for the supervised worker; `--org` limits cleanup to one organization. The optional PostgreSQL coordinator owns retention-job leases in distributed mode, while encrypted browser credentials and session states require explicit revocation or deletion. See [operations/cost-controls.md](../../operations/cost-controls.md) for managing spend, and [security/secrets-management.md](../../security/secrets-management.md) for key handling guidance.
