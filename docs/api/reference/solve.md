...
| `mode` | string | no | `flash`, `focus`, `dive` (planner chooses if omitted). `flash` uses provider snippets only; `focus` fetches up to three top results; `dive` fetches up to five. |
...
See [responses.md](../responses.md) and [citations.md](../citations.md) for the full answer/sources/citations shape. When no absolute URL is supplied, the search provider supplies candidates first: `flash` remains lightweight, while `focus` and `dive` perform bounded follow-up fetch-and-extract operations before ranking and synthesis. A failed follow-up fetch retains the provider result as a degraded, still-redacted candidate rather than making the entire solve fail.
