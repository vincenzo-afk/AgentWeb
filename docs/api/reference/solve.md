...
| `inputs` | object | no | Structured skill inputs; for an absolute-URL render or interaction task, may contain the bounded browser `actions` array and opaque `credential_id` or `session_state_id` references |
